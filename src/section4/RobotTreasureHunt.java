...
public class RobotTreasureHunt implements KeyEventDispatcher {
	...
	private void spaceBarWasPressed() {
		...
		if (robotXLocation == 730 && robotYLocation == 400) {
			// --make a pop-up tell the robot where to go next
			JOptionPane.showMessageDialog(null,
					"Go to the place no one dares to go. Click spacebar to find map once you are there.");
		}
			...
			if (robotXLocation == 210 && robotYLocation == 60) {
				JOptionPane.showMessageDialog(null,
						"Then head to the not lands and search. CLick spacebar to find treasure once you are there");
			} 
				// 9. If the robot is in the final location
				// --call the treasureFound() method

				if (robotXLocation == 450 && robotYLocation == 540) {
					treasureFound();
				}
